 ...
 * Author: ahmed
 *
 * Created on April 20, 2022, 5:06 PM
 */
...
volatile uint16 session_counter = 0; //session time indication
uint8 timeout_flag = FALSE; //stores if the session is still valid or timed out 

int main(void) {
    uint8 temperature = 0; //The average temperature of the room
    uint8 temp_ones = NOT_SELECTED; //The right digit of the Tempreture
    uint8 temp_tens = NOT_SELECTED; //The left digit of the Tempreture
    uint8 login_mode = NO_MODE; //Store the current mode admin or user or not logged in
    ...
    LED_vInit(USER_LED_PORT, USER_LED_PIN); //initializes the led of user
    ...
    SPI_vInitMaster(); //initialization  of SPI
   
                        /***** Printing Welcome screen *****/

    LCD_vSend_string("Welcome HOME");
    _delay_ms(1000); //delaying the system for the given time in (ms)
    LCD_clearscreen(); //remove all previously printed characters on the LCD and move the cursor to the first column of the first row
                        ...
                        /***** Setting Admin and User passwords *****/
    //Read the state of the the passwords of the admin and user if they are set or not set

    if ((EEPROM_ui8ReadByteFromAddress(ADMIN_PASS_STATUS_ADDRESS) != PASS_SET) || (EEPROM_ui8ReadByteFromAddress(User_PASS_STATUS_ADDRESS) != PASS_SET)) {
        ...
        uint8 pass_counter = 0; //the counter of the characters of the pass
        uint8 pass[PASS_SIZE] = {NOT_STORED, NOT_STORED, NOT_STORED, NOT_STORED}; //the array storing the pass
        while (pass_counter < PASS_SIZE)//loop till the user finish inserting the pass
        {
            ...
            LCD_vSend_char(PASSWORD_SYMBOL); // to display Password symbol (*)
            _delay_ms(100); 
            pass_counter++; 
        }
        EEPROM_vWriteBlockToAddress(EEPROM_ADMIN_ADDRESS, pass, PASS_SIZE); //save the  password as a block to the EEPROM
        EEPROM_vWriteByteToAddress(ADMIN_PASS_STATUS_ADDRESS, PASS_SET); //write the status of password as it is set
        LCD_clearscreen(); //remove all previously printed characters on the LCD and move the cursor to the first column of the first row
        LCD_vSend_string("Pass Saved");// show password saved message
        _delay_ms(500); 
        LCD_clearscreen(); //remove all previously printed characters on the LCD and move the cursor to the first column of the first row

                             /***** Setting user password *****/
        pass_counter = 0; //reset password counter which count the characters of the pass
        LCD_vSend_string("Set User Pass");//printing the set user password menu
        LCD_movecursor(2, 1);
        LCD_vSend_string("UserN Pass:"); 
        while (pass_counter < PASS_SIZE)//loop till the user finish inserting the pass
        {
            key_pressed = NOT_PRESSED; //return the variable that holds the pressed key from keypad to its initial value
            ...
            pass[pass_counter] = key_pressed; //add the entered character to the password array
            LCD_vSend_char(key_pressed); //print the entered character
            _delay_ms(CHARACTER_PREVIEW_TIME); 
            LCD_movecursor(2, 12 + pass_counter); //move the LCD cursor to the previous location to write the password symbol over the character
            LCD_vSend_char(PASSWORD_SYMBOL); //  to display Password symbol (*)
...
pass_counter++; //increase the characters count
        ...
        EEPROM_vWriteBlockToAddress(EEPROM_User_ADDRESS, pass, PASS_SIZE); //save the entire password as a block to the EEPROM
        EEPROM_vWriteByteToAddress(User_PASS_STATUS_ADDRESS, PASS_SET); //write the status of pass as it is set
        LCD_clearscreen(); //remove all previously printed characters on the LCD and move the cursor to the first column of the first row
    ...
    }//The end of if admin and user password is set
    else//this code of else run only if the system is not running for the first time (ADMIN and USER passwords are set)
    {
    ...
    while (1)//The start of the periodic code
    {
        ...
        {//if timeout flag was raised
            ...
            LED_vTurnOff(USER_LED_PORT, USER_LED_PIN); //turnoff the led of the user
        ...
        while (login_mode == NO_MODE)//The user can only leave the loop only in case of he was logged in as user or admin
        {
            ...
            if (key_pressed != CHECK_ADMIN_MODE && key_pressed != CHECK_USER_MODE) {
                LCD_clearscreen(); 
            ...
            switch (key_pressed) {
                ...
                case CHECK_ADMIN_MODE:
                    ...
                    /**** USER login *******/
                case CHECK_USER_MODE:
                    while (login_mode != USER) {
                        key_pressed = NOT_PRESSED; //return the variable that holds the pressed key from keypad to its initial value
                        LCD_clearscreen(); //remove all previously printed characters on the LCD and move the cursor to the first column of the first row
                        ...
                        EEPROM_vReadBlockFromAddress(EEPROM_User_ADDRESS, stored_pass, PASS_SIZE); //Save the entire password in the EEPROM
                        ...
                        if (ui8ComparePass(pass, stored_pass, PASS_SIZE) == TRUE)//in case of right password
                        {
                            ...
                            LCD_vSend_string("USER mode");
                            _delay_ms(500); //Halt the system for the given time in (ms)
                            LED_vTurnOn(USER_LED_PORT, USER_LED_PIN); //turn the led of gust mode that is connected to the master micro controller on
                        ...
                        } else//in case of wrong password
                        {
                            ...
                            if (pass_tries_count >= TRIES_ALLOWED)//if the condition of the block mode is true
                            {
                                EEPROM_vWriteByteToAddress(LOGIN_BLOCKED_ADDRESS, TRUE); //write to the EEPROM TRUE to the the block mode address
                                block_mode_flag = TRUE; //turn on block mode
                                break; //breaks the loop of insert user password #while(login_mode != USER)# line 228
                            }//end of if that check if the number of tries exceeds the maximum tries allowed
                        }//end of the case of wrong password
                    }//end of loop of user login
                    break; //break for CHECK_USER_MODE switch case
            }//end of switch
            ...
            switch (show_menu) {
                case MAIN_MENU:
                    do {
                        ...
                        } else if (login_mode == USER)//check login mode
                        {
                            LCD_vSend_string("3:Room3 4:Room4"); //this menu options only printed if the logged in user is a user
                        }
                        /*******************************************************/
                        ...
                        } else if (key_pressed == SELECT_ROOM4 && login_mode == USER)//If key pressed is 4 and the logged in user is guest
                        {
                ...
                case MORE_MENU:
                    do {
                        ...
                        {
                            show_menu = Door_MENU; //Set the next menu to be shown to TV menu
                ...
                case ROOM4_MENU:
                    ...
                    if (login_mode == USER)//in case of USER is logged in
                    {
                        show_menu = MAIN_MENU; //Set the next menu to be shown to main menu
